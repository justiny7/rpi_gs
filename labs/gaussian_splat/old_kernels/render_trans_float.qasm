...
.set out_b, ra14
.set out_addr, ra15
.set transmittance, rb23

.set pixel_buffer, ra16
# ra16 - ra31 are reserved

.set neg_half_M_LOG2E, rb24
.set alpha_threshold, rb25
.set trans_threshold, rb26
.set i255, rb27
...
mov alpha_threshold, 0.99
mov trans_threshold, 254.0 / 255.0
mov i255, 1.0 / 255.0
...
.macro render_tile
    ...
    mov r0, 0.0
    .rep row, 64
        # mov r0, 0xFF000000 # full transmittance
        # mov pixel_buffer+row, r0
        reg_to_vpm_vec16 r0, row
    .endr
    ...
    :1
        ...
        .rep row, 16
            ...
            # mov r3, (pixel_buffer+row).unpack8df
            # mov out_r, (pixel_buffer+row).unpack8cf
            # mov out_g, (pixel_buffer+row).unpack8bf
            # mov out_b, (pixel_buffer+row).unpack8af
            vpm_to_reg_vec16 r3, row * 4
            vpm_to_reg_vec16 out_r, row * 4 + 1
            vpm_to_reg_vec16 out_g, row * 4 + 2
            vpm_to_reg_vec16 out_b, row * 4 + 3

            # r0 stores alpha
            fmul r0, cur_opacity, r4
            # fmin r0, r0, alpha_threshold
            # fmin.setf -, r0, i255
            # mov.ifcc r0, 0.0

            # transmittance pruning
            # fmin.setf -, r3, trans_threshold
            # mov.ifc r0, 0.0

            # transmittance update
            fsub r1, 1.0, r3  # (1 - transmittance)
            fmul r0, r1, r0   # (1 - transmittance) * alpha = new contribution
            fadd r1, r3, r0   # old_transmittance + (1 - transmittance) * alpha
            mov pixel_buffer, r1
            
            # calc out_r
            fmul r1, cur_color_r, r0
            fadd r1, r1, out_r
            mov pixel_buffer+1, r1

            # calc out_g
            fmul r1, cur_color_g, r0
            fadd r1, r1, out_g
            mov pixel_buffer+2, r1

            # calc out_b
            fmul r1, cur_color_b, r0
            fadd r1, r1, out_b
            mov pixel_buffer+3, r1

            reg_to_vpm_vec16 pixel_buffer, row * 4
            reg_to_vpm_vec16 pixel_buffer+1, row * 4 + 1
            reg_to_vpm_vec16 pixel_buffer+2, row * 4 + 2
            reg_to_vpm_vec16 pixel_buffer+3, row * 4 + 3
        .endr
...
:2
    ...
    .rep row, 16
        ...
        vpm_to_reg_vec16 r1, row * 4 + 1
        mov.pack8csf pixel_buffer+row, r1  # converts [0.0, 1.0] to [0, 255] in bit 2
        vpm_to_reg_vec16 r1, row * 4 + 2
        mov.pack8bsf pixel_buffer+row, r1  # converts [0.0, 1.0] to [0, 255] in bit 2
        vpm_to_reg_vec16 r1, row * 4 + 3
        mov.pack8asf pixel_buffer+row, r1  # converts [0.0, 1.0] to [0, 255] in bit 2
