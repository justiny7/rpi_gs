...
.macro render_tile
    ...
    :1
        ...
        .rep row, 16
            ...
            vpm_to_reg_vec16 out_r, row * 3
            vpm_to_reg_vec16 out_g, row * 3 + 1
            vpm_to_reg_vec16 out_b, row * 3 + 2
            ...
            # mov.pack8bsf pixel_buffer+row, r1  # same op as above in bit 1

            # calc out_b
            fmul r1, r3, out_b
            fadd r1, r1, r2
            mov pixel_buffer+2, r1
            # mov.pack8asf pixel_buffer+row, r1  # same op as above in bit 0

            reg_to_vpm_vec16 pixel_buffer, row * 3
            reg_to_vpm_vec16 pixel_buffer+1, row * 3 + 1
            reg_to_vpm_vec16 pixel_buffer+2, row * 3 + 2
        .endr
...
:2
    ...
    .rep row, 16
        ...
        vpm_to_reg_vec16 r1, row * 3
        mov.pack8csf pixel_buffer+row, r1  # converts [0.0, 1.0] to [0, 255] in bit 2
        vpm_to_reg_vec16 r1, row * 3 + 1
        mov.pack8bsf pixel_buffer+row, r1  # converts [0.0, 1.0] to [0, 255] in bit 2
        vpm_to_reg_vec16 r1, row * 3 + 2
