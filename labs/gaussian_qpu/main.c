...
void precompute_gaussians_naive(Camera* c, GaussianK* g, ProjectedGaussianK* pg, uint32_t num_gaussians) {
    for (uint32_t j = 0; j < num_gaussians; j += 16) {
        for (uint32_t i = j; i < j + 16; i++) {
            Vec3 g_pos = { { g->pos_x[i], g->pos_y[i], g->pos_z[i] } };
            project_point(c, g_pos, &pg->depth[i], &pg->screen_x[i], &pg->screen_y[i]);

            Vec3 g_scale = { { g->scale_x[i], g->scale_y[i], g->scale_z[i] } };
            Vec4 g_rot = { { g->rot_x[i], g->rot_y[i], g->rot_z[i], g->rot_w[i] } };
            Mat3 cov3d = compute_cov3d(g_scale, g_rot);
            Vec3 cov2d = project_cov2d(g_pos, cov3d, c->w2c, c->fx, c->fy);

            cov2d.x += 0.3f;
            cov2d.z += 0.3f;

            Vec3 cov2d_inv = compute_cov2d_inverse(cov2d);
            pg->cov2d_inv_x[i] = cov2d_inv.x;
            pg->cov2d_inv_y[i] = cov2d_inv.y;
            pg->cov2d_inv_z[i] = cov2d_inv.z;

            Vec3 sh[16];
            for (int k = 0; k < 16; k++) {
                sh[k] = (Vec3) { { g->sh_x[k][i], g->sh_y[k][i], g->sh_z[k][i] } };
            }
            Vec3 color = eval_sh(g_pos, sh, c->pos);
            pg->color_r[i] = color.x;
            pg->color_g[i] = color.y;
            pg->color_b[i] = color.z;
        }
    }
}
...
void main() {
    ...
    /*
    t = sys_timer_get_usec();
    precompute_gaussians_naive(c, g, pg, num_gaussians);
    uint32_t naive_t = sys_timer_get_usec() - t;
    DEBUG_D(naive_t);
    */
